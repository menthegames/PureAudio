...
public class LibraryService
{
    ...
    // ============================================================
    //  Cache — delegated to LibraryCacheService
    // ============================================================

    /// <summary>
    /// Cache service for persisting/loading the library tree.
    /// </summary>
    public LibraryCacheService Cache { get; } = new();

    /// <summary>
    /// Save the current library tree and file metadata to a cache file.
    /// Delegated to LibraryCacheService.
    /// </summary>
    public void SaveCache() => Cache.SaveCache(this);

    /// <summary>
    /// Try to load the library from cache. Delegated to LibraryCacheService.
    /// </summary>
    public bool TryLoadCache() => Cache.TryLoadCache(this);

    // ============================================================
    //  Public helpers used by LibraryCacheService
    // ============================================================

    /// <summary>
    /// Get the full path to the sources.json file.
    /// Made public for use by LibraryCacheService.
    /// </summary>
    ...
    /// <summary>
    /// Add a file path to the global dedup set. Returns false if already present.
    /// Made public for use by LibraryCacheService.
    /// </summary>
    public bool AddFilePath(string path) => _allFilePaths.Add(path);

    /// <summary>
    /// Clear all tracked file paths. Made public for use by LibraryCacheService.
    /// </summary>
    public void ClearAllFilePaths() => _allFilePaths.Clear();
}
