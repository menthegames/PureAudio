...
# Read the DLL from the cloned repo
with open(r'f:\Dev\PureAudioV2\r8brain_temp\DLL\Win64\r8bsrc.dll', 'rb') as f:
    d = f.read()
...
if d[0:2] == b'MZ':
    ...
    for i in range(num_sec):
        ...
        print(f"  {name:8} vsize={vsize:6} vaddr={vaddr:#08x} rsize={rsize:6} rptr={rptr:#08x}")
        ...
        if name == '.edata':
            ...
            num_functions = struct.unpack('<I', edata[16:20])[0]
            ...
            if name_ptr_off:
                ...
                for j in range(num_names):
                    ...
                    noff = rva_to_off(nrv)
                    if noff:
                        fname = d[noff:noff+100].split(b'\x00')[0].decode('ascii', errors='replace')
                        print(f"    {j}: {fname}")
    ...
    if not has_edata:
        ...
        # Search for function names as strings
        for func in [b'r8b_create', b'r8b_process', b'r8b_clear', b'r8b_delete',
                     b'r8b_CreateResampler', b'r8b_Process', b'r8b_Clear', b'r8b_Delete']:
            idx = d.find(func)
            if idx >= 0:
